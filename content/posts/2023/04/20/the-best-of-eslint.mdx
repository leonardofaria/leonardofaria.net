...
Once we have the parseable content, we can analyze it and ditch what we don't want. Imagine we don't want a function called `sum` and this would be a very simple code to report and fix "the issue":
...
[eslint-plugin-react-hooks](https://www.npmjs.com/package/eslint-plugin-react-hooks) is extremely useful if you don't understand well hooks because it 'teaches' you how to use the feature introduced in React 16.8.

### eslint-plugin-jsx-a11y

[eslint-plugin-jsx-a11y](https://www.npmjs.com/package/eslint-plugin-jsx-a11y) checks for some acessibility rules in JSX elements. I previously [wrote about accessibility](https://leonardofaria.net/2020/08/13/automating-accessibility-tests-with-cypress) here and keep in mind that using linters and automated tests don't garantee that your app or website is 100% accessible however it can capture several mistakes, like [images without an alternative text](https://github.com/jsx-eslint/eslint-plugin-jsx-a11y/blob/main/src/rules/alt-text.js) or [pages without a `lang` attribute](https://github.com/jsx-eslint/eslint-plugin-jsx-a11y/blob/main/src/rules/lang.js).
...
[eslint-config-airbnb-base](https://www.npmjs.com/package/eslint-config-airbnb-base) is the right choice if you are not using React. It still gives you all JavaScript best-practices and style defined by Airbnb. In the past I have used this package in Ember.js applications.
...
[eslint-plugin-prettier](https://www.npmjs.com/package/eslint-plugin-prettier) is helpful to avoid ESLint conflicting with Prettier. Overall we should not rely on ESLint for formating our code and this plugin will make sure that ESLint doesn't get on Prettier's way.

### eslint-plugin-relay

[eslint-plugin-relay](https://www.npmjs.com/package/eslint-plugin-relay) catches common problems in code using Relay. I learned about this plugin at my current job and I can see how a [rule to remove unused fields](https://github.com/relayjs/eslint-plugin-relay/blob/main/src/rule-unused-fields.js) from your GraphQL queries can help with performance of your application.

### eslint-plugin-eslint-comments

[eslint-plugin-eslint-comments](https://www.npmjs.com/package/eslint-plugin-eslint-comments) applies best practices on directive comments such as `/* eslint-disable */`. Between many best practices, the recommended rules of this plugin require you to describe which rule do you want to disable.
...
In languages like Ruby we prefix variables with `_` if we are not using them. The rule as it is doesn't respect that so we can fix with:
...
This is a rule that I disable because I don't see the benefit of prefering default exports. Like mentioned in the Airbnb's [project issue](https://github.com/airbnb/javascript/issues/1365), this rule makes refactoring more difficult.
...
I like this rule because in my opinion it makes easier to find a prop in a component. I also like to move to the end shorthand and callback props:
